...
# year = [2020]
# k = 4
logger = SimpleLogger(f'../output/sample/gmm-clusters-k-{k}-year-{year}-{datetime.now()}.log')
logger.log("Start of GMM clustering run")
logger.log("Start of GMM clustering run with parameters: ")
...
#%%
logger.log(f"Applying GMM with k={k} to data with shape: {df.shape}")
model = GaussianMixture(n_components=k, max_iter=1000, n_init=2)
...
plt.title(f"PCA Visualization of GMM Clusters for k: {k} Year: {year}")
...
plt.legend()
# plt.show()
plt.savefig(f"../output/gmm-clusters-k-{k}-year-{year}.png")
...
#     print(f"Silhouette Score for k={k}: {s_score}")
#     s_scores.append(s_score)

# fig, (ax1, ax2) = plt.subplots(1, 2)
# # first plot: Silhouette score
# ax1.plot(n_components, s_scores)
# ax1.set_title('Silhouette Score')
# ax1.set(xlabel='Number of clusters', \
#     ylabel='Silhouette Score')

# # second plot: BIC
# ax2.plot(n_components, bic_scores)
# ax2.set_title('BIC')
# ax2.set(xlabel='Number of clusters', \
#     ylabel='BIC')

# plt.savefig(f"../output/gmm-bic-year-{year}.png")
...
logger.log("End of GMM clustering run")
